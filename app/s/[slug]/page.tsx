      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {products.length === 0 ? (
          <div className="text-center py-20">
            <p className="text-gray-500 text-lg">Nenhum produto disponível ainda.</p>
          </div>
        ) : (
                  ...
                  <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-100">
                    ...
                    <a
                      href={`https://wa.me/${store.whatsapp}?text=${encodeURIComponent(
                        `Olá, tenho interesse no produto: ${product.name} (${product.price.toLocaleString('pt-AO')} Kz). Ainda está disponível?`
                      )}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 transition-colors duration-200 text-sm font-medium"
                    >
